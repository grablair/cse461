						...
						try {
							...
							try {
								...
								while ( true ) {
									...
									int amountSent = 0;
									while (amountSent < transferSize) {
										int amountToSend = Math.min(transferSize - amountSent, 1000);
										tcpMessageHandlerSocket.sendMessage(Arrays.copyOfRange(data, amountSent, amountSent + amountToSend));
